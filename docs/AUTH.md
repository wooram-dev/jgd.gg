...
- 스토리 목록의 프로필 사진·표시 이름·게시/만료 시각은 비로그인 방문자에게도 공개한다. 목록은 사진 byte나 내부 userId를 포함하지 않는다. 표시용 스토리 이미지·썸네일은 로그인한 ACTIVE/BANNED 사용자만 조회한다. 원본을 제공하는 사용자 API는 없다.
- 게시는 ACTIVE 사용자만 가능하며 Origin과 DB 계정 상태를 확인한다. BANNED 작성자의 스토리는 열람 대상에서 제외한다.
- 사용자가 직접 선택해 제출한 사진 원본은 24시간 공개가 끝나도 서버에 보관한다. 보관 기간에 자동 만료는 없으며 계정 삭제 시 함께 삭제한다. 확인된 사진 삭제 문의는 `contact@jgd.gg`를 통해 운영자가 처리한다.
- 원본에는 EXIF/GPS 등 포함된 메타데이터가 남을 수 있다. 사용자에게 제공하는 이미지와 썸네일은 서버가 재인코딩해 메타데이터를 제거한다.
- 게시 전, 홈 스토리 영역과 로그인 개인정보 안내에 “24시간 뒤 숨김 · 원본 보관”을 고지한다. 게시·로그인 안내에는 목록의 프로필·이름이 비로그인 방문자에게도 공개됨을 알린다. 자동 삭제된다고 안내하지 않는다.
- 브라우저에는 내부 userId·원본 파일명·원본 byte를 보내지 않는다. 이미지 응답은 인증과 만료 검사 후 `private, no-store`로 반환한다.
