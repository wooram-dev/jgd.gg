...
4. 게임 궁합: 12문항·16유형 안내와 `/게임궁합` 진입 카드
5. 오늘의 기록: 실제 TOP 5, 오클릭과 최종 기록, 전체 랭킹 진입
6. 기다리는 동안, 잠깐 한 판: 기존 number-click 진입
7. 보조 열: 비로그인 Discord 연결 안내 또는 로그인 사용자 프로필·내 기록, 주간 랭킹, 펼칠 수 있는 이용 가이드
...
- 로그인 없으면 `/login`으로 이동하고 `returnTo`에 `/내정보`를 URL 인코딩해 전달
...
Discord 이름·아바타 편집은 없다. 게임 프로필은 요약 아래, 포인트 위에 배치하며 게임 선택·닉네임·선택 티어를 입력한다. [features/game-profiles.md](features/game-profiles.md)의 저장·취소·삭제 확인·오류 복구·개인정보 안내를 제공한다. 입력 label·focus ring·44px 이상 조작 영역을 제공하고 좁은 화면에서는 입력란을 한 열로 쌓는다.

`/멤버`는 공통 메뉴의 `게임 프로필`에서 진입한다. 로그인 후 멤버 확인이 끝나야 카드가 표시된다. 게임 필터, 내 프로필 등록·수정 링크, 다음 멤버 보기·처음으로를 제공한다. 카드에는 Discord 표시 이름·아바타와 각 게임의 닉네임·티어, 사용자 입력·미인증 안내, 수정 날짜를 표시한다. 긴 텍스트는 카드 안에서 줄바꿈한다. 프로필 조회 데이터는 페이지 HTML에 넣지 않고 보호 API에서 불러온다.

### 4.4 포인트

로그인 사용자의 공통 header에 확정 잔액을 `1,250 P`처럼 간결하게 표시하고, `/내정보`에는 현재 잔액과 최근 변동 내역을 제공한다. 비로그인 사용자에게 가상 잔액을 만들지 않는다.
...
- 칭호 상점은 공통 메뉴와 내 칭호 링크로 진입한다. 준비되지 않은 역할 설정은 판매 준비 중으로 표시하고 구매를 비활성화한다.
...
- [ ] 로그인 사용자는 header와 `/내정보`에서 확정 포인트를 확인하고 loading·빈 원장·오류·재시도를 구분한다.
- [ ] 포인트 적립·회수는 부호와 사유 텍스트를 함께 표시하며 게임 기록 점수와 혼동되지 않는다.

## 게임 성향·궁합 화면

공통 메뉴와 홈 카드에서 `/게임궁합`으로 진입한다. 공개 intro는 12문항·16유형, 네 축과 펼쳐 보는 유형 목록을 제공한다. 질문은 한 번에 하나씩 native radio로 선택하며 진행률·이전/다음 버튼을 표시한다. 미응답 시 다음 버튼은 비활성이다. 단계 이동과 결과 생성 후 제목으로 focus를 옮긴다. 결과는 네 글자 코드·별명·축별 선택 근거와 친구 유형 select를 제공한다. 친구 비교는 같은/다른 취향을 색과 텍스트로 함께 구분한다. 복사 상태를 live region으로 알리고 실패 시 직접 복사할 textarea를 제공한다. 320px에서 카드를 한 열로 쌓고 모든 선택·버튼은 44px 이상 조작 영역을 제공한다. 기능 계약은 [게임 궁합](features/game-compatibility.md)을 따른다.


## 칭호 상점과 내 칭호

`/상점`은 기존 dark theme 토큰과 카드·버튼 스타일을 사용한다. 여섯 칭호를 넓은 화면 두 열, 600px 이하 한 열로 배치한다. 상품·500P·소장 중·장착 중·잔액 부족을 텍스트로 구분하고 가격·잔액은 tabular 숫자를 쓴다. `/내정보#titles`는 소장 목록만 표시한다.

구매 확인은 native dialog로 이름·가격·예상 잔액·환불 불가를 안내하고 취소에 초기 focus를 둔다. Escape/취소는 원래 버튼으로 복귀하고, 성공은 제목 focus와 live region으로 알린다. mutation 동안 모든 변경 버튼을 잠그며 네트워크 실패는 같은 요청 재시도를 제공한다. 적용 대기는 실제 장착 완료와 구분한다. 성공 후 공통 header와 포인트 내역도 갱신한다. 상세 흐름은 [칭호 상점](features/title-shop.md)을 따른다.
